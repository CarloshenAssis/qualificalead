...
export class ApifyProspectingProvider implements ProspectingProvider {
 ...
 private mapRun(raw:unknown):ProviderRun{ const run=apifyRunSchema.parse(raw); return {runId:run.id,datasetId:run.defaultDatasetId??null,state:state(run.status),costCents:Math.ceil((run.usageTotalUsd??0)*100)}; }
}
