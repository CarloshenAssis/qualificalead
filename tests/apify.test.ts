...
describe('Apify adapter',()=>{
 ...
 it('autentica webhook e rejeita assinatura inválida',()=>{const secret='1234567890123456';expect(verifyApifyWebhook(`Bearer ${secret}`,secret)).toBe(true);expect(verifyApifyWebhook('Bearer bad',secret)).toBe(false);});
});
