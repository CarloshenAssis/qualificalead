...
describe('Apify adapter',()=>{
 ...
 it('autentica webhook e rejeita assinatura inválida',()=>{const body='{"x":1}',secret='1234567890123456';expect(verifyApifyWebhook(body,`Bearer ${secret}`,secret)).toBe(true);expect(verifyApifyWebhook(body,'Bearer bad',secret)).toBe(false);});
});
